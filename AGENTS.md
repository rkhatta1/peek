...
- convex is a beast best tamed. often refer to the `convex-performance-audit` skill to verify the changes you make to the codebase, especially convex code.
...
Always use the shadcn MCP server for both shadcn/ui component information and component usage examples.

Before adding, modifying, or composing a shadcn/ui component:

1. Consult shadcn MCP for the current component API and implementation guidance.
2. Consult shadcn MCP for relevant usage examples and composition patterns.
3. Follow the repository's existing `components.json`, theme tokens, aliases, and local component conventions.
4. Prefer existing components under `components/ui/` before installing another copy.

Do not rely only on model memory for shadcn/ui APIs or examples when the shadcn MCP server is available.

## Git Discipline

- Commit the changes whenever a slice of work or a phase of plan is implemented AND reviewed by the user.
- Aim for one commit per Plane task and vice versa.
...
**Note: `./docs/` contains tracked project documentation, not agent working
memory. Start with `./docs/README.md`: implementation-specific material belongs
in `./docs/codebase/`, while stable business terms, lifecycle semantics,
formulas, and metrics belong in `./docs/glossary/`. If business logic changes,
update the canonical glossary definition first and then its codebase consumers.
Be strict with CRUD in docs because they are lifecycle sources of truth.**
